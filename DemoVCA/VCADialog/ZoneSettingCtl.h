...
public:
	...
	void	OnChangeClientRect(RECT rect){m_ClientRect = rect;}
	void	OnCommand(WPARAM wParam, LPARAM lParam);

	void	FireOnEvent(DWORD uiEventType, DWORD uiContext);

	void	AddZone(BOOL bZone);
	void	RemoveZone();
	void	AddNode();
	void	RemoveNode();
	void	AddCounter();
...
private:
	...
	void			GetMouseClipFromArea( HWND hWnd, unsigned int uiTotalPoints, VCA5_POINT pPoints[], CPoint cpPoint, CRect *rcClient);
