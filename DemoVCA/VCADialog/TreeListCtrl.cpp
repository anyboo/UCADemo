...
// Global handle to the Windows Message hook
HHOOK   g_hHook   =   NULL;   
...
LRESULT CALLBACK GetMessageProc( int nCode, WPARAM wParam, LPARAM lParam )
{
	...
	if ( ( nCode >= 0) && PM_REMOVE == wParam && (lpMsg->message >= WM_KEYFIRST && lpMsg->message <= WM_KEYLAST) )   
	{
		...
		if ( ( lpMsg->wParam == VK_BACK ) || ( lpMsg->wParam == VK_LEFT ) || ( lpMsg->wParam == VK_RIGHT ) /*|| ( lpMsg->wParam == VK_RETURN )*/ )
		{			
			int id = GetDlgCtrlID( lpMsg->hwnd );
			HWND hParent = GetParent( lpMsg->hwnd );
...
//#define IDC_TREELIST	123
			...
			{
//				MSG tMsg = *lpMsg;
//				SendMessage( tMsg.hwnd, tMsg.message, tMsg.wParam, tMsg.lParam );
//				TreeList_KeyPressed( lpMsg->hwnd, lpMsg->wParam, lpMsg->lParam );
//				lpMsg->message   =   WM_NULL;
//				lpMsg->lParam   =   0L;
//				lpMsg->wParam   =   0;
//				AfxGetApp()->PreTranslateMessage((LPMSG)lParam );
			}
			else if ( ( idparent == IDC_TREELIST ) && ( lpMsg->lParam > 0 ) )
			{															  
				MSG tMsg = *lpMsg;
				SendMessage( hParent, WM_KEYDOWNEDIT, tMsg.wParam, tMsg.lParam );
//				TCHAR	tDebug[200];
//				wsprintf( tDebug, _T("KeydownEdit,wParam=0x%x,lParam=0x%x\n"), tMsg.wParam, tMsg.lParam );
//				OutputDebugString( tDebug );
				lpMsg->message  =   WM_NULL;
				lpMsg->lParam   =   0L;
				lpMsg->wParam   =   0;
			}
...
BOOL CTreeListCtrl::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pWnd != NULL) 
	{
		...
		{
//			TreeList_CallBackProc( m_hWnd, pMsg->message, pMsg->wParam, pMsg->lParam );
...
// BW - When we die, make sure the class gets unregistered
CTreeListCtrl::~CTreeListCtrl()
{
    // Uninstall the WH_GETMESSAGE hook function.   
	if (g_hHook) {
		VERIFY( ::UnhookWindowsHookEx( g_hHook ) );   
		g_hHook = NULL;
	}
...
BOOL CTreeListCtrl::Create(DWORD dwStyle, const RECT &sRect, CWnd* pParentWnd,UINT nId)
{
	...
	BOOL fRet = CWnd::Create(_T("TreeList"),NULL,dwStyle,sRect,pParentWnd,nId);
...
//				  Use -1 for an auto expanding column.
//				  Use values below -1 for predefined variable columns.	
