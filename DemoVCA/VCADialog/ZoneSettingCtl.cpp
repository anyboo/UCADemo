...
void CZoneSettingCtl::CreateMenu()
{
	...
	m_hSelectedZoneMenu		= CreatePopupMenu();
	m_hSelectedZoneMenu_WithOutNode	= CreatePopupMenu();
	m_hSelectedCounterMenu	= CreatePopupMenu();

	m_hMenuZone = CreatePopupMenu();
	AppendMenu( m_hMenuZone, MF_STRING,		ID_ADDZONE,		_T("Create zone" ));
	AppendMenu( m_hMenuZone, MF_STRING,		ID_ADDLINE,		_T("Create line" ));
	AppendMenu( m_hMenuZone, MF_SEPARATOR,  10000,			_T("" ));
	AppendMenu( m_hMenuZone, MF_STRING,		ID_REMOVEZONES,	_T("Remove All zones/lines" ));
	//AppendMenu( m_hMenuZone, MF_STRING,		ID_REMOVEZONES,	_T("Remove all zones/counters" ));

	// Submenu for node
	m_hMenuNode = CreatePopupMenu();
	AppendMenu( m_hMenuNode, MF_STRING,	ID_ADDNODE,		_T("Insert node" ));
	AppendMenu( m_hMenuNode, MF_STRING,	ID_REMOVENODE,	_T("Remove node" ));

	// Submenu for counters
	m_hMenuCounter = CreatePopupMenu();
	AppendMenu( m_hMenuCounter, MF_STRING,	ID_ADDCOUNTER,		_T("Add counter" ));
	AppendMenu( m_hMenuCounter, MF_STRING,	ID_RESETCOUNTERS,	_T("Reset All counters" ));
	AppendMenu( m_hMenuCounter, MF_STRING,	ID_REMOVECOUNTERS,	_T("Remove All counters" ));

	// Submenu for rules
	m_hMenuRule = CreatePopupMenu();
	AppendMenu( m_hMenuRule, MF_STRING, ID_PRESENCE, _T("Presence" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_ENTER, _T("Enter" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_EXIT, _T("Exit" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_APPEAR, _T("Appear" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_DISAPPEAR, _T("Disappear" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_STOP, _T("Stopped" ));	
	AppendMenu( m_hMenuRule, MF_STRING, ID_DWELL, _T("Dwell" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_DIRECTION, _T("Directional Filter" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_SPEED, _T("Speed" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_TAILGATING, _T("Tailgating" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_ABANDONREMOVE, _T("Abandoned/Removed Object" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_COLOR_FILTER, _T("Color Filter" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_SMOKE, _T("Smoke Detection" ));
	AppendMenu( m_hMenuRule, MF_STRING, ID_FIRE, _T("Fire Detection" ));

	// BW - Menu for display options
	m_hMenuDisplay	= CreatePopupMenu();
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_BLOBS, _T("Display Blobs" ));
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_NON_ALARMS, _T("Display Non-Alarmed Objects" ));
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_TARGETID, _T("Display Target ID" ));
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_OBJECTHEIGHT, _T("Display Object Height" ));
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_OBJECTSPEED, _T("Display Object Speed" ));
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_OBJECTAREA, _T("Display Object Area" ));
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_OBJECTCLASSIFICATION, _T("Display Object Classification" ));
	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_OBJECTCOLORSIGNATURE, _T("Display Object Color Signature" ));
//	AppendMenu( m_hMenuDisplay, MF_STRING, ID_DISPLAY_TAMPER_ALARM, _T("Display Tamper Info" ));
	
	// Main menu
	AppendMenu( m_hMainMenu, MF_POPUP,	(UINT)m_hMenuZone, _T("Zone/Line" ));
	AppendMenu( m_hMainMenu, MF_POPUP,	(UINT)m_hMenuCounter, _T("Counter" ));
	AppendMenu( m_hMainMenu, MF_SEPARATOR,  10000,			_T("" ));
	AppendMenu( m_hMainMenu, MF_POPUP,	(UINT)m_hMenuDisplay, _T("Display" ));

#ifdef NVC_CONFIGURE
	AppendMenu( m_hMainMenu, MF_SEPARATOR,  10000,			"" );
	AppendMenu( m_hMainMenu, MF_STRING,	ID_SAVE_NVE_CONFIGURE, "Save NVE Configure" );
#endif 	

	m_hMenuSelectedZone = CreatePopupMenu();
	
	AppendMenu( m_hMenuSelectedZone, MF_STRING,		ID_ADDZONE,		_T("Create zone" ));
	AppendMenu( m_hMenuSelectedZone, MF_STRING,		ID_ADDLINE,		_T("Create line" ));
	AppendMenu( m_hMenuSelectedZone, MF_SEPARATOR,  10000,			_T("" ));
	AppendMenu( m_hMenuSelectedZone, MF_STRING,		ID_REMOVEZONE,	_T("Remove zone/line" ));

	// Selected Zone Menu
	AppendMenu( m_hSelectedZoneMenu, MF_POPUP,	(UINT)m_hMenuSelectedZone, _T("Zone/Line" ));
	AppendMenu( m_hSelectedZoneMenu, MF_POPUP,	(UINT)m_hMenuNode, _T("Node" ));
	AppendMenu( m_hSelectedZoneMenu, MF_POPUP,	(UINT)m_hMenuRule, _T("Rules" ));

	// Selected Zone Menu without Node
	AppendMenu( m_hSelectedZoneMenu_WithOutNode, MF_POPUP,	(UINT)m_hMenuSelectedZone, _T("Zone/Line" ));
	AppendMenu( m_hSelectedZoneMenu_WithOutNode, MF_POPUP,	(UINT)m_hMenuRule, _T("Rules" ));

	// Selected Counter Menu
//	AppendMenu( m_hSelectedCounterMenu, MF_STRING,	ID_ADDCOUNTER, _T("Add counter" ));
	AppendMenu( m_hSelectedCounterMenu, MF_STRING,	ID_RESETCOUNTER, _T("Reset counter" ));
	AppendMenu( m_hSelectedCounterMenu, MF_STRING,	ID_REMOVECOUNTER, _T("Remove counter" ));


}


BOOL CZoneSettingCtl::Setup(CVCADialog *pVCADialog)
{
	if(m_bSetup){
...
//	PixelToPercent(&PreviewPerPoint);
	...
	if (VCA5_APP_AREA_T_ZONE == m_eActiveAreaType && NOT_IN_USE != m_nActiveNode) {
		...
		if( !ValidateZone( m_nActiveAreaIdx ) )
		{
			// Put it back
			m_pDataMgr->ChangeNode( m_nActiveAreaIdx, m_nActiveNode, &oldPt );
		}
...
void CZoneSettingCtl::ShowMenu(POINT point, UINT uiDisplayMenuHiliteItem)
{
	...
	if(NOT_IN_USE == m_nActiveAreaIdx){
		...
		DWORD eng = m_pVCADialog->GetEngine()->GetEngId();
...
//		EnableMenuItem(m_hMenuDisplay, ID_DISPLAY_TAMPER_ALARM, (ulFunction&VCA5_FEATURE_PRESENCE)? MF_ENABLED:MF_DISABLED|MF_GRAYED);
	...
	TrackPopupMenu(hShowMenu, TPM_LEFTALIGN | TPM_TOPALIGN, point.x, point.y, 0, m_hWnd, NULL);
}
...
void CZoneSettingCtl::AddZone(BOOL bZone)
{
	...
	if(bZone)
		...
		m_nActiveAreaIdx = m_pDataMgr->AddLine(&point);
...
void CZoneSettingCtl::ResetCounter()
{
	...
	VCA5_APP_COUNTERS*	pCounters = m_pDataMgr->GetCounters();
	((CVCAEngine *)m_pVCADialog->GetEngine())->SetControl(VCA5_CMD_SETCOUNTERRES, pCounters->pCounters[m_nActiveAreaIdx].usCounterId, 0);
}
...
void CZoneSettingCtl::ResetCounters()
{
	...
	for(ULONG i = 0 ; i < pCounters->ulTotalCounters ; i++){
		((CVCAEngine *)m_pVCADialog->GetEngine())->SetControl(VCA5_CMD_SETCOUNTERRES, pCounters->pCounters[i].usCounterId, 0);
	}
}
	...
	if ( (  pRuleA && pRuleA->ucTicked ) ||  (  pRuleB && pRuleB->ucTicked )){
		...
		for (UINT i = 0; i < pZone->ulTotalPoints-1; i++)
		{
			...
			for (int n = 0; n < 2; n++)
			{
				...
				ptTop.x = (ULONG)round(ptMiddle2.x + 25  * cos( dAngle ));
				ptTop.y = (ULONG)round(ptMiddle2.y - 25  * sin( dAngle ));
				//ptTop.y = (ULONG)round(ptMiddle2.y + 25  * sin( dAngle ));

				ptBottom.x = (ULONG)round(ptMiddle2.x - 25  * cos( dAngle ));
...
void CZoneSettingCtl::GetMouseClipFromArea( HWND hWnd, unsigned int uiTotalPoints, VCA5_POINT pPoints[], CPoint cpPoint, CRect *rcClient)
{
