...
#pragma comment(lib,"./Render/VCAMetaRender.lib")
...
void	CVCAD3DRender::OnChangeClientRect(RECT rect)
{
	...
	if (0 != ClientRect.Width() && 0 != ClientRect.Height()) {
		m_ClientRect = rect;
		m_bDraw = TRUE;
	} else {
		m_bDraw = FALSE;
...
void	CVCAD3DRender::DrawSingleCounter(RECT rcVCAROI, VCA5_APP_COUNTER *pCounter)
{
	...
	int height = CRect(&m_ClientRect).Height();
	float	fScalor = 0.0f;
	UINT uiFontSize = 0;
	...
	for (ULONG i = 0; i < Polygon.cnt_point; ++i) {
		PERCENTTOPIXEL(Polygon.atpoint[i].x, pCounter->pPoints[i].x, rcVCAROI1.Width());
		PERCENTTOPIXEL(Polygon.atpoint[i].y, pCounter->pPoints[i].y, rcVCAROI1.Height());
		Polygon.atpoint[i].x += rcVCAROI1.left;
		Polygon.atpoint[i].y += rcVCAROI1.top;
	}
...
void CVCAD3DRender::DrawColorSignature(RECT rcVCAROI, VCA5_PACKET_OBJECT *pObject, BYTE alpha, COLORREF color)
{
	...
	PERCENTTOPIXEL( ptBoundaryLeftTop.y, (pObject->bBox.y - pObject->bBox.h/2), rcVCAROI1.Height());
	PERCENTTOPIXEL( ptBoundaryLeftTop.x, (pObject->bBox.x + pObject->bBox.w/2), rcVCAROI1.Width());

	ptBoundaryLeftTop.y = max(0, ptBoundaryLeftTop.y);
	ptBoundaryLeftTop.x = min(rcVCAROI1.Width()-1, ptBoundaryLeftTop.x);
...
void	CVCAD3DRender::DrawTargetID(RECT rcVCAROI, VCA5_PACKET_OBJECT *pObject, COLORREF color)
{
	...
	PERCENTTOPIXEL( ptStartXY.x, (pObject->bBox.x - pObject->bBox.w/2), rcVCAROI1.Width());
	PERCENTTOPIXEL( ptStartXY.y, (pObject->bBox.y - pObject->bBox.h/2), rcVCAROI1.Height());
...
void	CVCAD3DRender::DrawObjectInformation(RECT rcVCAROI, VCA5_PACKET_OBJECT *pObject, BYTE alpha, COLORREF Color)
{
	...
	PERCENTTOPIXEL( Xmin, (pObject->bBox.x - pObject->bBox.w/2), rcVCAROI1.Width());
	PERCENTTOPIXEL( Ymin, (pObject->bBox.y - pObject->bBox.h/2), rcVCAROI1.Height());
...
void	CVCAD3DRender::DrawTrail(RECT rcVCAROI, VCA5_PACKET_OBJECT *pObject, BYTE alpha, COLORREF Color, BOOL bFromCenter)
{
	...
	PERCENTTOPIXEL( Xmin, (pObject->bBox.x - pObject->bBox.w/2), rcVCAROI1.Width());
	PERCENTTOPIXEL( Xmax, (pObject->bBox.x + pObject->bBox.w/2), rcVCAROI1.Width());
	PERCENTTOPIXEL( Ymin, (pObject->bBox.y - pObject->bBox.h/2), rcVCAROI1.Height());
	PERCENTTOPIXEL( Ymax, (pObject->bBox.y + pObject->bBox.h/2), rcVCAROI1.Height());
	...
	{
		PERCENTTOPIXEL( uiTrailX[i], pObject->trail.trailPoints[i].x, rcVCAROI1.Width() );
		PERCENTTOPIXEL( uiTrailY[i], pObject->trail.trailPoints[i].y, rcVCAROI1.Height() );
