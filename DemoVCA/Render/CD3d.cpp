...
BOOL CD3d::InitBrushTexture( eBRUSHTYPE brushType, UINT bitmapId )
{
	...
	HBITMAP hBmp = LoadBitmap( AfxGetInstanceHandle(), MAKEINTRESOURCE( bitmapId ) );
	ASSERT( hBmp );
...
BOOL CD3d::InitGroundTexture()
{
	...
	// Load the bitmap
	HBITMAP hBmp = LoadBitmap( AfxGetInstanceHandle(), MAKEINTRESOURCE( IDB_GROUND ) );
	if( hBmp == NULL)
	{
		TCHAR buf[100];
...
BOOL CD3d::LoadTextureFromResource(LPDIRECTDRAWSURFACE7 *ppddsTexture, DWORD idRes, DWORD bmpsize)
{
	...
	D3DX_SURFACEFORMAT fmt = D3DX_SF_R8G8B8; 
		...
		// Load the bitmap
	HBITMAP hBmp = LoadBitmap( AfxGetInstanceHandle(), MAKEINTRESOURCE( idRes ) );
	ASSERT( hBmp );
