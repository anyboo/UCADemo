...
{
	m_VCARender.Setup(hWnd, pDataMgr);
...
void CCalibrationD3DRender::DrawGrid()
{
	...
	if(m_pDataMgr->m_uiCalibDrawOptions&VCA_CALIB_RULER_DRAW){
		...
		if (rulerlength >= 0.0f){
			DrawRulerMeasure(rulerlength, meaUnits, CPoint(m_pDataMgr->m_LastClickedMousePos));
		}
	}
}


void CCalibrationD3DRender::DrawRulerMeasure(float fLength, int meaUnits, CPoint ptStartMsg)
{
	char	string[128] = {0,};
